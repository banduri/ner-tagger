...
the '/api/nernosplit' endpoint will try to process the text without splitting at the cost of high (V)RAM usage.
    ...
    parser.add_argument('--zmqsplittimeout', type = int, nargs='+',
                        default = [5000,10000,30000],
                        help = "the timeouts for the split-server in milliseconds before every retry - more timeouts, means more retries")
    ...
    parser.add_argument('--zmqmiddlewaretimeout', type = int, nargs='+',
                        default = [5000,10000,30000],
                        help = "the timeouts for the middleware-server in milliseconds before every retry - more timeouts, means more retries")
